...
function App() {
  ...
  const [devInfo, setDevInfo] = useLocalStorage('devInfo', {});
  const initialDevInfo = {};
  const [userName, setUserName] = useLocalStorage('userName', ''); //useState('')
  const [theme, setTheme] = useLocalStorage('theme', 'dark');
  const [icon, setIcon] = useState(theme === 'dark' ? sunSvg : moonSvg)

  const toggleTheme = () => {
    setTheme((curr) => (curr === 'light' ? 'dark' : 'light'))
    setIcon(theme === 'dark' ? moonSvg : sunSvg);
  }
  ...
  return (
    <DevInfoContext.Provider value={{ devInfo, setDevInfo }}>
      <MyContext.Provider value={{ theme, toggleTheme, icon, handleUserName, userName, handleKeyDown, handleSubmit, handleReset }}>
